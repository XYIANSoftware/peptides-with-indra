...
- **OG and Twitter images**: Link previews (texts, iMessage, social) use the dedicated share image: `public/main-app-twitter-og-meta-image-icon.png` is copied to `public/og-image.png` and referenced in layout as `openGraph.images` and `twitter.images` (1536×1024). With `metadataBase`, URLs are absolute so crawlers and messengers show the image.
